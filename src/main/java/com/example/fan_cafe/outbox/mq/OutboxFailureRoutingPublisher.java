...
public class OutboxFailureRoutingPublisher {
    ...
    /**
     * DLQ 적재 시 운영 화면 조회용 스냅샷을 DB에도 함께 기록한다.
     * 브로커에 없어도 DB 이력으로 사고 원인을 분석하고 수동 재처리할 수 있다.
     */
    ...
    /**
     * 자동 복구 불가·재시도 상한 초과 메시지를 DLQ 브로커 큐에 적재하고,
     * 동시에 DB({@code dlq_events})와 Slack 모두에 기록한다.
     */
    public void publishToDlq(String payload, int retryCount, String errorMessage, DlqRoutingType routingType) {
        ...
        dlqService.persistFromMqPublish(payload, retryCount, errorMessage, routingType);
